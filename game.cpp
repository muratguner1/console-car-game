...
void *newGame(void *) {
    ...
    while (playingGame.IsGameRunning) {
        key = getch();
        if (key != KEYERROR) {
            if (key == playingGame.leftKey && playingGame.current.x >4) { // if press left button
                drawCar(playingGame.current, 1,1);
                playingGame.current.x -= playingGame.current.speed; // update position to left
                drawCar(playingGame.current, 2,1);
                ...
                playingGame.current.x += playingGame.current.speed; // update position right
...
void *enqueueCar(void *) { //add car to queue
    while (playingGame.IsGameRunning) {
        if (playingGame.cars.size() <= maxCarNumber -1) { 
            ...
            c.height = MINH + rand() % 3;
            c.width = MINW + rand() % 3;

            c.speed = c.height /2;
            do{
                c.x = MINX + rand() % 86;
                if((c.x + c.width < lineX || c.x > lineX) && c.x + c.width < wWidth -3)
                    break;
            }while(1);
            c.y = -(rand() % 11);
...
void *dequeueCar(void*) //send car to moveCar method
{
    ...
    while (playingGame.IsGameRunning) {
       Car c;
        ...
        pthread_create(&thMove, NULL, moveCar, (void *)&c);

        int rndSleep = rand() % DeQueueSleepMin;
...
void *moveCar(void* car) //allows the movement of the car 
{
    Car c =  *(Car*)car;
    while (playingGame.IsGameRunning && c.isExist && !playingGame.IsSaveClicked){
        ...
        if (c.y > EXITY) {
            drawCar(c, 1, 0);
            c.isExist = false;      
            playingGame.points += c.height * c.width;
            writePoint();

            if (playingGame.points >= playingGame.level * levelBound && playingGame.level < MAXSLEVEL) {
                playingGame.level++;
            ...
            if(checkCollision(playingGame.current, c)){
                c.isExist = false; 
                playingGame.IsGameRunning = false; 
                FILE *writePoint = fopen(pointsTxt, "a+");
...
void pointsScreen() {//print pont to the screen
    clear();
...
void saveGame()
{
    ...
    while(!playingGame.serhat.empty())
    {
